  ...
  private
    ...
    def golden_five_routes
      ...
      route <<-RUBY.gsub(/^      /, "")
        ...
        match("/#{plural_table_name}/:the_#{singular_table_name}_id", { :controller => "#{plural_table_name}", :action => "show", :via => "get"})
        
        # UPDATE
        
        match("/modify_#{singular_table_name}/:the_#{singular_table_name}_id", { :controller => "#{plural_table_name}", :action => "update", :via => "#{skip_post? ? "get" : "post"}"})
        
        # DELETE
        match("/delete_#{singular_table_name}/:the_#{singular_table_name}_id", { :controller => "#{plural_table_name}", :action => "destroy", :via => "get"})
